...
class HuffTree
{
    ...
    public:
        ...
        HuffTree    (char character, int weight, int height) {
...
inline void balanceProcess  (HuffNode *&root, int count = 3)
{
    while (count > 0)
    {
        if (root->isLeaf() == true) {
            ...
            if (abs(difference) <= 1) {
                if (root->isLeaf() == false) {
                    InternalNode *rt = dynamic_cast<InternalNode*>(root);
                    balanceProcess(rt->getLeftNode(), count);
                    balanceProcess(rt->getRightNode(), count);
                }
                return;
            } 
...
inline int getHeight(HuffNode *root)
{
...
// cout << "Input: " << input << endl << endl;       
        ...
        for (int i = 0; i < tmp; i++) {
            HuffTree *newNode = new HuffTree(listNameFreq[i].first, listNameFreq[i].second, 0);
