...
public:
	...
	// Trả về số lượng khách bị kick
	{
		int numberOfCusKicked = 0;
	...
	// Trả về số lượng khách bị kick
	{
		...
		tmpCus = orderTableCus;
		while (tmpCus->next != nullptr) {
			tmpCus = tmpCus->next;
		}
		for (int i = 0; i < totalOrderTableCus; i++) {
			if ((option == 1 && tmpCus->energy > 0) || (option == 2 && tmpCus->energy < 0)) {
				pop_TableCus(tmpCus->name);
				numberOfCusKicked++;
				i--;
	...
	bool is_identicalName 				(const string name)
	{
		...
		for (int i = 0; i < totalCusQueue; i++) {
			if (name == tmpCus1->name) {
				return true;
			}
			tmpCus1 = tmpCus1->next;
		}
		tmpCus = nullptr;
		tmpCus1 = nullptr;
		return false;
	...
	void printEliminatedCus			(const int option, int numberOfCusKicked)
	{
		Restaurant::customer *tmpCus = tmpAnotherCus;
	...
	void DOMAIN_EXPANSION() override
	{
		...
		int numberOfCusKicked;
		if (sumPostive >= sumNegative) {
			numberOfCusKicked = kickCus(2);
			printEliminatedCus(2, numberOfCusKicked);
		}
		else if (sumPostive < sumNegative) {
			numberOfCusKicked = kickCus(1);
			printEliminatedCus(1, numberOfCusKicked);
		}
